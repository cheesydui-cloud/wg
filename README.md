...
当前版本：**v4.6.6**（主入口可改 · 单码分享 · Agent 版本判定修复 · 侧栏两字 · 主题提亮）
...
> **正确路径（已打通验证）**  
> **电脑/客户端 → 商家 IX 前置（拓扑可配置 Host）→ IX(DNAT) → 落地家宽 mita → 出网**  
>
> - 「主入口」= 拓扑里填写的商家前置 Host（IP/域名，**可随时更换**），**不是手机、不是 IX 本机**  
> - 客户端 Endpoint = 主入口，**不是**家宽公网 IP、不是面板 IP  
...
### v4.6.6
- **第二台 IX 转发脚本串台修复**：禁止 `getIx`/焦点落地回落到第一台；无绑定时明确报错不生成 7901 脚本；脚本头带 `ixId`/端口段；UI 按 IX 缓存脚本。

### v4.6.5
- **Agent 更新修复**：心跳下发 `agentTargetVersion`（脚本版本），不再用面板 UI 4.6.x 误导自更新；Agent 升至 **4.3.3**（路径探测/原子写入/强制更新）。
- **诊断**：去掉误报的全局「商家前置入口」项；每台 IX 检查 Host 是否填写。
- **拓扑前置 UI**：选择卡直接显示 Host；保存强制校验；空 Host 回落与每 IX Endpoint 端口修正。

### v4.6.4
- **第二台 IX 通网**：落地端口自动纠正到所属 IX 段；用户 `route.ixId` 跟落地；分享链前置按落地 IX；转发脚本含用户专用端口。
- 换落地/换 IX 时对齐端口与用户绑定；诊断不再把未绑落地误算进第二台 IX。

### v4.6.3

- **端口段按 IX 校验**：诊断不再写死 7900–7999；每台 IX 用自己的 `portMin–portMax`（如 10400–10499）。
- **用户专用端口随落地/IX**：编辑用户默认留空=落地端口；切换落地自动对齐段；段外 7901 会拒存并在诊断标红；启动时自动清掉段外专用端口。

### v4.6.2

- **拓扑转发脚本常显**：不再藏在默认折叠的「进阶设置」里；顶栏可一键跳到「转发脚本」。

### v4.6.1

- **主入口可改**：拓扑不再写死 211/114 产品 IP；主/备用/自定义 Host 任意填写与切换。
- **分享只保留主入口**一条二维码/链接（不再默认双码展示备用前置）。
- **Agent 过旧判定**：对比仓库 `agent` 脚本版本（如 4.3.2），**不再**用面板 UI 版本（4.6.x）误伤。
- **侧栏导航统一两字**：概览 / 拓扑 / 落地 / 客户 / 诊断 / 设置。
- **主题提亮**：Soft slate 运维台，降低 OLED 纯黑刺眼。
- 现有 state 里已填的前置 IP **原样保留**；仅新建默认不再塞示例 IP。
...
- **Premium Ops Console（前端）**：按 ui-ux-pro-max 设计系统重做视觉——OLED 深色底 `#05070d`、玻璃侧栏、Bento 模块、Inter + JetBrains Mono、主色 `#5B8CFF` + cyan 点缀、指标卡光晕、路径条/表格/弹窗/登录页统一高级运维台风格。
- 默认强制 dark；亮色仅保留兼容 token。
- **仅面板需升到 4.5.0**；Agent 逻辑无改动，仍可用 4.3.2。

### v4.4.1

- **视觉重做（前端）**：深色侧栏 + 渐变主区背景；修复概览 **指标卡塌成纯文字**（补全 `.stat-grid`/`.stat-card`）；按钮/路径条/模块头更有层次与阴影；亮色主题也保持深色导航。
- 概览指标卡增加副标题与状态色。

### v4.4.0

- **模块化工作台 UI（前端）**：设计 token + 模块卡片（`.mod`）；客户端页改为 **落地页签 + 紧凑表 + 行展开 + 筛选搜索**；分享弹窗 **扫码 / 链接 / 配置** 三分 Tab（YAML/JSON 在配置页）；概览与拓扑视觉对齐。
- 保留 4.3.x 能力：禁用下发、下载 YAML、约 3s 实时网速、到期日期选择。
- **仅面板需升到 4.4.x**；Agent 逻辑无改动，仍可用 4.3.2（已装可不升）。

### v4.3.2

- **禁用真正生效**：点禁用后服务端立即排队 apply；bundle 的 `serverConfig.users` 只含启用账号；Agent 强制剔除禁用用户并 `user-disable` 兜底；本落地全部禁用时用 `panelhold` 占位覆盖 mita（真实用户无法认证）。**请升级面板 + 落地 Agent 到 4.3.2**。
- 修：此前仅改面板徽章、未可靠下发，或落地「无启用用户」时 apply 被 400 拒绝，导致仍能连。

### v4.3.1

- **下载 YAML**：分享弹窗一键导出完整 OpenClash/mihomo 配置（`type: mieru` + 211/114 双节点 + DNS/分组/前置 DIRECT 防环），按客户账密生成，导入即用。
- **禁用/启用**：客户端卡片按钮；禁用后自动 apply 本落地。
- **实时网速**：客户端页约 3s 静默刷新，局部更新上下行数字（有弹窗不整页闪）。
- **到期日**：编辑用户改为日期选择 +「不限」+ 快捷 +7/+30/+90/+1 年。

### v4.3.0

- **UI 收敛（仅前端）**：顶栏只保留一条主告警，其余收进「另有 N 项待办」；多落地入口条默认折叠；概览改为紧凑路径 + 双栏快捷列表；落地详情主操作 / Agent·危险折叠分区；全局 mita 默认参数折叠。文案统一：**只连前置，勿连 IX / 家宽**。
- 不改服务端 / Agent 业务逻辑；Agent 版本号对齐 4.3.0。
...
打开 `http://面板IP:51821`，确认版本 **v4.6.6**。
